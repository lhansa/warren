...
cartera_warren0 = {
    'sym' : ['AAPL', 'BAC', 'KO', 'AXP', 'KHC', 'MCO'], 
    'name' : ['Apple', 'Bank of America', 'Coca Cola', 
              'American Express', 'Kraft  Heinz', 'Moodys'],
    'perc' : [0.4418, 0.1085, 0.0883, 0.0713, 0.0514, 0.0335],
    'shares' : [980622264, 925008600,400000000, 
                151610700, 325634818, 24669778]
}
...
#%% Exportación para clase
df_cartera = pd.DataFrame().from_dict(cartera_warren0)

df_todo.loc[:,['close']].to_csv('output/evolution_values.csv')
df_todo_wide.to_csv('output/evolution_values_wide.csv')
df_cartera.to_csv('output/warren_portfolio.csv')
